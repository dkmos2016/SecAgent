...
import SecAgent.SecAgent_Core.utils.ReqInfo;
...
public abstract class CommonStub extends AdviceAdapter implements Opcodes {
  ...
  protected void classLoaderInfo() {
    ...
    mv.visitMethodInsn(
        INVOKESTATIC,
        "SecAgent/SecAgent_Core/Logger/ExceptionLogger",
        "doExpLogger",
  ...
  protected void getGlobalReqInfo(int reqinfo_idx) {
    ...
    findAndExecute("SecAgent.SecAgent_Core.utils.ReqLocal", "getReqInfo", null, null_idx, null_idx, reqinfo_idx);
  }
  ...
  protected void findAndExecute(
    ...
    newArrayList(params_idx);
    addListElement(params_idx, T_OBJECT, tmp_obj);
    loadClass("SecAgent.SecAgent_Core.Logger.ExceptionLogger", cls_idx);
  ...
  protected void findAndGetSecProxyInstance(int obj_idx, int dst_idx) {
    ...
    loadClass("SecAgent.SecAgent_Core.Filter.SecInstanceProxyFactory", cls_idx);
